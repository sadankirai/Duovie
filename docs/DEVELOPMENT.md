...
The development diagnostics area (visible once connected, not hidden behind a control) now shows a "Connection Quality" panel: selected ICE path (e.g. `srflx → srflx`), transport/relay protocol, RTT, and role-appropriate metrics — Host sees capture/encoded resolution, send bitrate, FPS, packet loss, quality-limitation reason, and codec; Guest sees received resolution, receive bitrate, FPS, frames dropped, packet loss, jitter, and codec. Unavailable values show `—`. This is observability only; no non-authoritative quality label (Excellent/Good/Poor) was added — the task's own guidance was to omit an arbitrary threshold-based label when real-world calibration data doesn't yet justify one, and raw metrics are more useful for diagnosis.

## Motion-optimized screen-share encoding baseline (Stage 7.2)

A real moving-video cross-network acceptance test (Mac Host on ordinary Wi-Fi, phone Guest on cellular, `iceTransportPolicy: "all"`, selected pair `srflx ↔ srflx`) used the Stage 7.1 telemetry to diagnose the reported "watchable but periodically blurry/stuttery" moving video. Findings: **0% packet loss** and **low jitter** in that observation — the network path itself was healthy — while **`qualityLimitationReason: "bandwidth"`** was reported at roughly 1.4–1.6 Mbps, with Duovie sending the native captured resolution and no explicit encoding policy at all. Cloudflare TURN/Tunnel was not in the media path (the selected pair was direct `srflx ↔ srflx`), so it is not the bottleneck.

Task 7.2 responds with a fixed, non-adaptive **baseline**, applied once per Host capture (`frontend/src/features/peer/screenShareEncoding.ts`), entirely through `RTCRtpSender` parameters — never DOM/canvas resizing, never SDP munging, never renegotiation, and never a full adaptive-quality system:

- **Target resolution:** approximately 720p30 when the captured source is larger than that, computed by `calculateScaleResolutionDownBy(width, height)` fitting the source within a 1280×720 bounding box (`Math.max(1, width/1280, height/720)`) — this preserves aspect ratio for any source shape, never upscales a smaller capture, and safely no-ops (`1`) on missing/invalid dimensions. 1920×1080 → scale ≈1.5 → ≈1280×720; 2560×1440 → scale ≈2 → ≈1280×720.
- **Frame rate:** `maxFramerate: 30` where supported — a stable 30 FPS baseline, not 60 FPS.
- **Content hint:** `track.contentHint = "motion"` where supported (feature-detected; a failure never fails sharing).
- **Degradation preference:** `degradationPreference: "maintain-framerate"` where supported, so WebRTC's congestion control prefers to sacrifice resolution over frame rate under constraint — matching the "smooth motion over native resolution" product goal for screen-share viewing.
- **Bitrate ceiling:** `maxBitrate: 3_500_000` (3.5 Mbps) — the midpoint of a reasonable 3.0–4.0 Mbps 720p30 range. This is a **ceiling only**, giving headroom above the ~1.6 Mbps observed on a constrained cellular path; it is not a guaranteed/minimum bitrate, and congestion control remains fully authoritative below it.

The whole `sender.setParameters(...)` call is wrapped in one try/catch: any unsupported field or browser quirk (desktop Chrome, Edge, and Safari remain the targets, and not every `RTCRtpSender` supports every parameter identically) degrades silently to today's plain screen-share behavior rather than failing the share. The profile is applied immediately after the existing pre-negotiated sender's `replaceTrack` succeeds — the same ordering and no-renegotiation architecture as before — so repeat sharing naturally recalculates from each new capture's actual settings. Stop Sharing, native Chrome Stop, peer/Hub recovery, capture cleanup, and refresh continuity are all unchanged.

This is explicitly **not** Stage 7.3: no automatic quality ladder, no Excellent/Good/Poor-driven changes, no hysteresis, no bitrate-based automatic resolution switching, no 1080p auto-upgrade, no user quality selector, and no 60 FPS mode. A follow-up real phone/cellular re-test with this baseline in place, reading the extended Stage 7.1 telemetry (capture vs. encoded vs. received resolution), will determine whether and how Stage 7.3 automatic adaptation is justified.

...
The development diagnostics UI renders `RoomRuntimeSnapshot.qualitySnapshot` as a read-only "Connection Quality" panel (selected ICE path, transport, RTT, and role-appropriate sender/receiver metrics, `—` when unavailable)—observability only. `OutboundVideoQuality`/`InboundVideoQuality` also expose non-sensitive dimensions: Host `captureWidth`/`captureHeight` (from the local capture track's `getSettings()`) and `encodedWidth`/`encodedHeight` (from `outbound-rtp.frameWidth`/`frameHeight`, the actual sent resolution after any sender-side scaling); Guest `frameWidth`/`frameHeight` (from `inbound-rtp`, the actual received/decoded resolution). These let a manual test directly compare native capture against what is actually encoded/received.

## Motion-optimized screen-share encoding baseline (Stage 7.2)

A real moving-video cross-network acceptance test (Mac Host on Wi-Fi, phone Guest on cellular, direct `srflx ↔ srflx` P2P) used the Stage 7.1 telemetry above and found the connection itself healthy—0% packet loss, low jitter, stable 30 FPS—while `qualityLimitationReason: "bandwidth"` was reported at roughly 1.4–1.6 Mbps: Duovie was sending the native captured resolution (`getDisplayMedia({ video: true, audio: false })`, no constraints) with no sender-side scaling, framerate cap, content hint, or bitrate ceiling. Moving content periodically went blurry with occasional stutter.

`frontend/src/features/peer/screenShareEncoding.ts` adds a fixed, non-adaptive motion-oriented baseline applied once per Host capture, entirely through `RTCRtpSender` parameters—never DOM/video-element resizing, canvas re-encoding, renegotiation, or SDP munging. `calculateScaleResolutionDownBy(width, height)` is a small pure function: it fits the captured `MediaTrackSettings` width/height within a 1280×720 bounding box via `Math.max(1, width/1280, height/720)`, which preserves aspect ratio for any source shape (not assumed 16:9), never upscales (minimum result `1`), and safely returns `1` for missing/invalid dimensions. `applyMotionOptimizedSenderProfile(sender, track)` then best-effort sets `track.contentHint = "motion"` (feature-detected, try/caught) and calls `sender.setParameters(...)` with that `scaleResolutionDownBy`, `maxFramerate: 30`, `maxBitrate: 3_500_000` (3.5 Mbps—the midpoint of a 3.0–4.0 Mbps 720p30 range, giving headroom above the ~1.6 Mbps observed on a constrained cellular path without an unbounded encode; a ceiling only, never a guaranteed/minimum bitrate—congestion control remains authoritative), and `degradationPreference: "maintain-framerate"`. The whole `setParameters` call is wrapped in one try/catch: any unsupported field or browser quirk degrades silently to today's plain screen-share behavior rather than failing the share.

`WebRtcPeerController.startScreenShare()` preserves its existing pre-negotiated-sender/`replaceTrack`-without-renegotiation architecture; the profile is applied immediately after `replaceTrack` succeeds, using the sender and the just-acquired track, so repeat sharing naturally recalculates from each new capture's actual settings. Stop Sharing, native Chrome Stop, peer/Hub recovery, capture cleanup, and refresh continuity are all unchanged—this only adds one best-effort step between an already-succeeded `replaceTrack` and the existing "active" state transition. No automatic quality ladder, hysteresis, bitrate-driven resolution switching, or user quality selector exists yet; those remain a possible Stage 7.3 decision pending a follow-up real phone/cellular test with this baseline in place.
